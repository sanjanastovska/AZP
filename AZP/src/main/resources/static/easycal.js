...
(function($, window, document, _, moment, undefined){
	...
	var Easycal = {
		...
		_attachEventHandlers : function(){
			...
			self.$elem.find('table.' + classes.timeGridTable + ' .' + classes.timeSlot).on('click', function(ev){
				if($(ev.target).closest('.' + classes.eventContainer).length || $(ev.target).hasClass(classes.eventContainer)){
					...
					var slotStartTime = $(this).attr('data-date-time')
		...
		_renderTimeGridHTML : function(){
			...
			var cellContent = null, timeTag = null, colDate = null, dateTimeTag=null;
				...
				for(;time.isBefore(maxTime);){
					if(i === 0){
						...
						dateTime.subtract(1,'d');
                                                var dateTimeString = dateTime.format(format.dateShort) + " " +timeTag;
						
						// timeTag = time.format(this.options.dateLong);
						html += '<div class="table-cell ' + classes.timeSlot + '" data-time="' + timeTag + '" data-date-time="' + dateTimeString + '">';
